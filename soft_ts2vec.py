    ...
    def fit(self, train_data, train_labels, test_data, test_labels, soft_labels, run_dir, n_epochs=None, n_iters=None, verbose=False):
        ...
        while True:
            ...
            for x, idx in train_loader:
                ...
                # take take_per_row returns A_i = [off_i + el: off_i + r] intervals
                out1_all = self._net(take_per_row(x, crop_offset + crop_eleft, crop_right - crop_eleft))
                

                # take take_per_row returns B_i = [off_i + l: off_i + er] intervals
                # (intersection of A & B) = [off_i + l: off_i + r] where el<l , r < er
                out2_all = self._net(take_per_row(x, crop_offset + crop_left, crop_eright - crop_left))
                out1 = out1_all[:, -crop_l:] #take only ovelapping representations [off_i + l: off_i + r]
                out2 = out2_all[:, :crop_l]  #take only ovelapping representations [off_i + l: off_i + r]
                # porblem
                loss = hier_CL_soft(
                    out1,
                    out2,
